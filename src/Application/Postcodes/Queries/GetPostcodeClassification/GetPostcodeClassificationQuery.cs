...
public class GetPostcodeClassificationQueryHandler(IApplicationDbContext context) : IRequestHandler<GetPostcodeClassificationQuery, PostcodeResult>
{
    #region Fields
    ...
    public async Task<PostcodeResult> Handle(GetPostcodeClassificationQuery request, CancellationToken cancellationToken)
    {
        ...
        var suburb = await _context.Suburbs.Where(s => s.Name.Replace(" ", "").ToLower() == request.Suburb.Replace(" ", "").ToLower())
                                         .AsNoTracking()
                                        .FirstOrDefaultAsync(cancellationToken) ?? throw new NotFoundException(request.Suburb, nameof(Suburb));

        var postcode = await _context.PostcodeSuburbMapper.Where(psm => psm.PostcodeSuburbMapper_SuburbID == suburb.ID)
